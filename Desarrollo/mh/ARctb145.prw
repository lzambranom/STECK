...
oSection1 := TRSection():New(oReport,cSayCusto,{"cArqTmp","CTT"},/*aOrder*/,/*lLoadCells*/,/*lLoadOrder*/)
...
oSection1:SetHeaderPage()
	...
	TRCell():New(oSection1,"CUSTO"	,"cArqTmp",cSayCusto				,/*Picture*/,12		,/*lPixel*/,bCdCUSTO)
	TRCell():New(oSection1,"CCRES"	,"cArqTmp",STR0027	,/*Picture*/,aTamCCRes[1]	,/*lPixel*/,bCdCCRES)
	TRCell():New(oSection1,"DESCCC"	,"cArqTmp",STR0028	,/*Picture*/,nTamCC			,/*lPixel*/,/*{|| }*/)
	TRCell():New(oSection1,"SALDOANT","cArqTmp",STR0029 	,/*Picture*/,TAM_VALOR	,/*lPixel*/,{|| ValorCTB(cArqTmp->SALDOANT ,,,TAM_VALOR-2,nDecimais,.T.,cPicture,CTT->CTT_NORMAL,,,,,,lPrintZero,.F.)},"RIGHT",,"RIGHT")// Saldo Anterior
	TRCell():New(oSection1,"SALDODEB","cArqTmp",STR0030 	,/*Picture*/,TAM_VALOR	,/*lPixel*/,{|| ValorCTB(cArqTmp->SALDODEB ,,,TAM_VALOR,nDecimais,.F.,cPicture,CTT->CTT_NORMAL,,,,,,lPrintZero,.F.)},"RIGHT",,"RIGHT")// Debito
	TRCell():New(oSection1,"SALDOCRD","cArqTmp",STR0031 	,/*Picture*/,TAM_VALOR	,/*lPixel*/,{|| ValorCTB(cArqTmp->SALDOCRD ,,,TAM_VALOR,nDecimais,.F.,cPicture,If(CTT->CTT_NORMAL = "0",CT1->CT1_NORMAL,CTT->CTT_NORMAL),,,,,,lPrintZero,.F.)},"RIGHT",,"RIGHT")// Credito

	If lMov //Imprime Coluna Movimento!!
		TRCell():New(oSection1,"MOVIMENTO","cArqTmp",STR0032 ,/*Picture*/,TAM_VALOR	,/*lPixel*/,{|| ValorCTB(cArqTmp->MOVIMENTO,,,TAM_VALOR-2,nDecimais,.T.,cPicture,If(CTT->CTT_NORMAL = "0",CT1->CT1_NORMAL,CTT->CTT_NORMAL),,,,,,lPrintZero,.F.)},"RIGHT",,"RIGHT")// Movimento do Periodo

	EndIf

	TRCell():New(oSection1,"SALDOATU"	,"cArqTmp",STR0033 			,/*Picture*/,TAM_VALOR	,/*lPixel*/,{|| ValorCTB(cArqTmp->SALDOATU ,,,TAM_VALOR-2,nDecimais,.T.,cPicture,If(CTT->CTT_NORMAL = "0",CT1->CT1_NORMAL,CTT->CTT_NORMAL),,,,,,lPrintZero,.F.)},"RIGHT",,"RIGHT")// Saldo Atual
 	TRCell():New(oSection1,"TIPOCC"		,"cArqTmp",STR0034 	+" "+cSayCusto	,/*Picture*/,01			,/*lPixel*/,/*{|| }*/)// Centro Custo / Sintetica           
 	TRCell():New(oSection1,"TIPOCONTA"	,"cArqTmp",STR0034	+" "+STR0025	,/*Picture*/,01			,/*lPixel*/,/*{|| }*/)// Conta Analitica / Sintetica           
	...
	oSection1:Cell("TIPOCONTA"	):Disable()
	...
	oSection1:OnPrintLine( {|| ( IIf( lPula .And. (cTipoAnt == "1" .Or. (cArqTmp->TIPOCC == "1" .And. cTipoAnt == "2")), oReport:SkipLine(),NIL),;
									 cTipoAnt := cArqTmp->TIPOCC;
								)  })
	
	oSection1:SetLineCondition({|| f145Fil(cSegAte,cSegmento,nDigitAte,cSegIni,cSegFim,cFiltSegm,nPos,nDigitos,cMascara1) })

// Totais das sessoes	
	oBreak:= TRBreak():New(oSection1,{ || cArqTmp->CONTA },STR0020,.F.)
	
 	oBreak:OnBreak({ || nTotdbt := oTRF1:GetValue(),nTotcrt := oTRF2:GetValue() })   

 	oTRF1 := TRFunction():New(oSection1:Cell("SALDODEB"),nil,"SUM",oBreak,/*Titulo*/,/*cPicture*/,{ || f145Soma("D",cSegAte) },.F.,.F.,.F.,oSection1)
 	oTRF1:disable()
 	 		 TRFunction():New(oSection1:Cell("SALDODEB"),nil,"ONPRINT",oBreak,/*Titulo*/,/*cPicture*/,{ || ValorCTB(nTotdbt,,,TAM_VALOR,nDecimais,.F.,cPicture,"1",,,,,,lPrintZero,.F.) },.F.,.F.,.F.,oSection1)
	
	oTRF2 := TRFunction():New(oSection1:Cell("SALDOCRD"),nil,"SUM",oBreak,/*Titulo*/,/*cPicture*/,{ || f145Soma("C",cSegAte) },.F.,.F.,.F.,oSection1)
	oTRF2:disable()
		 	 TRFunction():New(oSection1:Cell("SALDOCRD"),nil,"ONPRINT",oBreak,/*Titulo*/,/*cPicture*/,{ || ValorCTB(nTotcrt,,,TAM_VALOR,nDecimais,.F.,cPicture,"2",,,,,,lPrintZero,.F.) },.F.,.F.,.F.,oSection1)

  	If lMov
 	    TRFunction():New(oSection1:Cell("MOVIMENTO"),nil,"ONPRINT",oBreak,/*Titulo*/,/*cPicture*/,{ || ( (nTotMov := nTotCrt-nTotDbt),;
 		ValorCTB(nTotMov,,,TAM_VALOR-2,nDecimais,.T.,cPicture,CT1->CT1_NORMAL,,,,,,lPrintZero,.F.)  )},.F.,.F.,.F.,oSection1)
	EndIf


// Total geral

	oTRF3 := TRFunction():New(oSection1:Cell("SALDODEB"),nil,"SUM",/*oBreak*/,/*Titulo*/,/*cPicture*/,{ || f145Soma("D",cSegAte) },.F.,.F.,.F.,oSection1)
	oTRF3:disable()
			 TRFunction():New(oSection1:Cell("SALDODEB"),nil,"ONPRINT",/*oBreak*/,/*Titulo*/,/*cPicture*/,{ || ValorCTB(oTRF3:GetValue(),,,TAM_VALOR,nDecimais,.F.,cPicture,"1",,,,,,lPrintZero,.F.) },.F.,.T.,.F.,oSection1)

	oTRF4 := TRFunction():New(oSection1:Cell("SALDOCRD"),nil,"SUM",/*oBreak*/,/*Titulo*/,/*cPicture*/,{ || f145Soma("C",cSegAte) },.F.,.F.,.F.,oSection1)
	oTRF4:disable()
			 TRFunction():New(oSection1:Cell("SALDOCRD"),nil,"ONPRINT",/*oBreak*/,/*Titulo*/,/*cPicture*/,{ || ValorCTB(oTRF4:GetValue(),,,TAM_VALOR,nDecimais,.F.,cPicture,"2",,,,,,lPrintZero,.F.) },.F.,.T.,.F.,oSection1)
...
Local nDigitos		:= 0
	...
	If !( RecCount() == 0 .And. !Empty(aSetOfBook[5]) )
	 ...
	 //	oSection1:SetParentFilter( { |cParam| cArqTmp->CONTA == cParam },{ || cArqTmp->CONTA })// SERVE PARA IMPRIMIR O TITULO DA SECAO PAI
