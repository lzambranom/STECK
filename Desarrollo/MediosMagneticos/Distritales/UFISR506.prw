...
If Pergunte( "FISR506" , .T. )
	...
	If !lAutomato
		MsgMeter({| oMeter, oText, oDlg, lEnd | ;
			U_UCtSalPlanRef(oMeter, oText, oDlg, @lEnd, @cArqRef, dDataIni, dDataFim, cPlanoRef, cVersao,;
	...
	If nCount > 0
		...
		If !lAutomato
			MsgMeter({| oMeter, oText, oDlg, lEnd | U_UGrabaTxt(oMeter, oText, oDlg, @lEnd, cDir, cArqRef, nAnio, nMonto)},;
				OemToAnsi(OemToAnsi(STR0005)),;  	//"Grabando archivo magnético ..."
				OemToAnsi(STR0001))  				//"Ingresos por actividades e ingresos exentos de ICA"
		Else
			U_UGrabaTxt(oMeter, oText, oDlg, @lEnd, cDir, cArqRef, nAnio, nMonto)
		EndIf
...
/*/{Protheus.doc} CtSalPlanRef
	...
	@return  Nil
/*/
Static Function UCtSalPlanRef(oMeter, oText, oDlg, lEnd, cArqTrab, dDataIni, dDataFim, cPlanRef, cVersao, cMoeda, cTpSald, lImpAntLP, dDataLP, nDivide, lVlrZerado, aSelfil)
Local cQuery 		:= ""
Local cAliasCVD		:= ""
Local aCampos		:= {}
...
/*/{Protheus.doc} GrabaTxt
	...
	@return  Nil
/*/
Static Function UGrabaTxt(oMeter, oText, oDlg, lEnd, cDir, cArqTmp, nAnio, nMonto)
Local nHdl		:= 0
Local cArq		:= ""
Local cLin		:= ""
Local cSep		:= ";"
